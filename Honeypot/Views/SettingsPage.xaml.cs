...
public sealed partial class SettingsPage : Page
{
    ...
    private async void SetWindowsHelloEnable(bool on)
    {
        try
        {
            if (MainViewModel.Instance.AppSettings.LockWithWindowsHello != on)
            {
                ...
                switch (await Windows.Security.Credentials.UI.UserConsentVerifier.RequestVerificationAsync(resourceLoader.GetString("UnlockAppUnlockingMessage")))
                {
                    case Windows.Security.Credentials.UI.UserConsentVerificationResult.Verified:
                        MainViewModel.Instance.AppSettings.LockWithWindowsHello = on;
                        break;
                    case Windows.Security.Credentials.UI.UserConsentVerificationResult.DeviceNotPresent:
                    case Windows.Security.Credentials.UI.UserConsentVerificationResult.NotConfiguredForUser:
                    case Windows.Security.Credentials.UI.UserConsentVerificationResult.DisabledByPolicy:
                        WindowsHelloToggleSwitch.IsOn = MainViewModel.Instance.AppSettings.LockWithWindowsHello;
